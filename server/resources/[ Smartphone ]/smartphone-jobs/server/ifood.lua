...
vRP.prepare('ifood/insert', [[INSERT INTO smartphone_ifood_orders
  (id, Passport, worker_id, store_id, total, fee, created_at)
  VALUES (@id, @Passport, @worker_id, @store_id, @total, @fee, UNIX_TIMESTAMP())
]])
vRP.prepare('ifood/search', 'SELECT * FROM smartphone_ifood_orders WHERE Passport=@Passport ORDER BY created_at DESC LIMIT 20')
...
function cancelOrder(order, refund)
  ...
  if refund then
    ...
    vRP.giveBankMoney(vRP.Passport(order.worker), order.fee)
...
expose('ifood_get_orders', function(source)
  ...
  return vRP.query('ifood/search', { Passport = Passport })
end)

expose('ifood_set_rate', function(source, id, rate)
  local Passport = vRP.Passport(source)
  assert(Passport, 'Você está bugado')
...
expose('ifood_create_order', function(source, id, cart, distance)
  ...
  local Passport = assert(vRP.Passport(source), 'Você está bugado (sem id)')

  local bank = toInt(vRP.GetBank(source))
  assert(bank >= total, 'Saldo bancário insuficiente')
  vRP.RemoveBank(Passport, total)
  ...
  CreateThread(function()
    ...
    if not order.worker then
      orders[order.id] = nil
      vRP.giveBankMoney(Passport, order.total)
    end
  end)
...
expose('ifood_finish_route', function(source, code)
  ...
  for idname, amount in pairs(order.items) do
    vRP.GenerateItem(order.customer.id, idname, amount, true)
  end
  vRP.giveBankMoney(Passport, order.fee)
...
AddEventHandler('smartphone:isReady', function()
  exports.smartphone:createApp(
    'ifood', 
    'iFood', 
    'nui://smartphone/assets/apps/ifood.webp',
