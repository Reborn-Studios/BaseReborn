...
vRP.prepare('uber/insert', [[INSERT INTO smartphone_uber_trips
  (`id`, `Passport`, `driver_id`, `total`, `from`, `to`, `created_at`, `finished_at`) VALUES 
  (@id, @Passport, @driver_id, @total, @from, @to, @created_at, UNIX_TIMESTAMP())
]])
...
expose('uber_create_trip', function(source, pos, distance)
  ...
  CreateThread(function()
    ...
    if not trip.driver then
      trips[trip.id] = nil
      vRP.giveBankMoney(Passport, trip.total)
    end
  end)
end)
...
AddEventHandler('smartphone:isReady', function()
  exports.smartphone:createApp(
    'uber',
    'Uber',
    'nui://smartphone/assets/apps/uber.webp',
