...
local mesasRoletas = Config.RulettTables
tableObjects = {}
...
createRulettAsztal = function(index, data)
    ...
    table.insert(tableObjects,self.tableObject)
            ...
            Citizen.CreateThread(function()
                while selectedRulett ~= nil do
                    ...
                    if idleTimer ~= nil then
                        SendNUIMessage({ action = "showTimer", timeLeft = idleTimer })
                ...
                function()
                    while selectedRulett ~= nil do
                        ...
                        if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                            local tmpInput = getGenericTextInputrolet("Coloque sua aposta")
    ...
    self.loadTableData = function()
        ...
        --('Table data successfully created..')
    end
    ...
    self.betRenderState = function(state)
        ...
        --('Bet rendering turned: %s', enabledBetRender)
    ...
    self.loadTableData()
    --(string.format('Rulett table created %s id', self.index))
...
function getGenericTextInputrolet(type)
    if type == nil then
        type = ''
    end
...
Citizen.CreateThread(function()
    ...
    for rulettIndex, data in pairs(mesasRoletas) do
            ...
            for k, v in pairs(Rulettek) do
                if DoesEntityExist(v.tableObject) then
                    ...
                    if dist < 2.5 then
                        ...
                        local closestChairData = getClosestChairDatarolet(v.tableObject)
...
AddEventHandler('client_callback:rulett:taskSitDown', function(rulettIndex, chairData)
    ...
    SetNuiFocus(true,false)
    startRuletta(rulettIndex, chairData.chairId)
    Citizen.Wait(4000)
    --SetPlayerControl(PlayerId(), 1, 0)
    SetNuiFocus(false,false)
end)

function startRuletta(index, chairId)
    ...
    if selectedRulett == rulettIndex then
        if not statusz then
            SendNUIMessage({
                game = "Roleta",
                action = 'setGameInfo',
                header = "Começando em instantes...",
                szoveg = string.format('%s %s', ido, "segundos")
            })
        else
            SendNUIMessage({
                game = "Roleta",
                action = 'setGameInfo',
                header = "Jogo em progresso",
                szoveg = nil
            })
        end
    end
...
function getClosestChairDatarolet(tableObject)
