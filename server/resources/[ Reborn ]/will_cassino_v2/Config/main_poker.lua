...
Main.DealerAnimDictShared = 'anim_casino_b@amb@casino@games@shared@dealer@' --
Main.DealerAnimDictPoker = 'anim_casino_b@amb@casino@games@threecardpoker@dealer'
Main.PlayerAnimDictShared = 'anim_casino_b@amb@casino@games@shared@player@'
Main.PlayerAnimDictPoker = 'anim_casino_b@amb@casino@games@threecardpoker@player'

Main.IncreaseAmounts = function(currentAmount)
...
Main.GetCardType = function(cardArrayId)
...
Main.GetCardValue = function(cardArrayId)
    local vals = {
        -- 2
        [2] = 2,
        [15] = 2,
...
Main.getHandAllValues = function(handTable, bool_1, bool_2)
    if type(handTable) == 'table' then
        local c1, c2, c3 = Main.GetCardValue(handTable[1]), Main.GetCardValue(handTable[2]), Main.GetCardValue(handTable[3])
        ...
        if (c1 ~= c2 and c1 ~= c3) and c2 ~= c3 then
            ...
            if Flush then
                ...
                if Main.GetCardType(handTable[1]) == Main.GetCardType(handTable[2]) and Main.GetCardType(handTable[1]) == Main.GetCardType(handTable[3]) then
        ...
        elseif c1 == c2 and c1 == c3 then -- 3 of a kind
            return c1 + c2 + c3 + 400
        elseif Main.GetCardType(handTable[1]) == Main.GetCardType(handTable[2]) and Main.GetCardType(handTable[1]) == Main.GetCardType(handTable[3]) then
            handValue = 200
        end
...
Main.GetPairMultiplier = function(handValue)
...
Main.GetAnteMultiplier = function(handValue)
    if handValue > 500 then
        return 5
    elseif handValue > 400 then
        return 4
...
Main.canDealerPlay = function(handValue)
