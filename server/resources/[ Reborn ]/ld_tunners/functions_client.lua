...
Notification = function( message)
    TriggerEvent("Notify","importante",message)
end
...
setVehicleProperties = function(vehicle, props)
    ...
    if DoesEntityExist(vehicle) and props then
        local preservedHeight, preservePlate
        if AirSuspension then
            preservePlate = AirSuspension.getVehiclePlate(vehicle)
            if preservePlate and AirSuspension.hasInstalled(preservePlate) then
                preservedHeight = AirSuspension.getActiveHeight(vehicle, preservePlate)
            end
        end
        ...
        if preservedHeight ~= nil and AirSuspension then
            AirSuspension.restoreHeight(vehicle, preservePlate, preservedHeight)
        end
    end
end
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("ld_tunners:server:requestVehData")
end)

function startMechanicLocationsThread()
    Citizen.CreateThread(function()
        while true do
            local sleep = 1000
            ...
            if IsPedInAnyVehicle(playerPed) then
                for mName, mData in pairs(Config.Locations) do
                    for i, mCoords in pairs(mData.coords) do
                        ...
                        if dst <= 20 then
                            sleep = 1
                            DrawMarker(36, mCoords.x, mCoords.y, mCoords.z,0,0,0,0,0,0,1.001,3.001,1.001,227, 227, 227,155,0,0,0,1)
                            if dst <= 6 then
                                if not menuOpened then
                                    if IsControlJustPressed(0, 38) and not menuOpened and (mData.permission == nil or vSERVER.hasPermission(mData.permission)) then
                                        menuOpened = true
