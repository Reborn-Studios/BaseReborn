...
vRP.prepare("vRP/get_group_hierarchy","SELECT * FROM permissions WHERE user_id = @user_id AND permiss = @permiss AND hierarchy = @hierarchy")
vRP.prepare("vRP/add_group","INSERT INTO permissions(user_id,permiss,hierarchy) VALUES(@user_id,@permiss,@hierarchy)")
