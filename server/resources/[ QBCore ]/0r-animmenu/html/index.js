...
window.addEventListener('message', function(event) {
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27 && animPosOpen) {
			animPosOpen = false;
            $("#animPosInfoDiv").css({right: "2%", position:'absolute', display: 'flex'}).animate({right: "-10%"}, 400, function() {
...
function clFunc(name1, name2, name3, name4, name5, name6) {
	if (name1 === "chooseCategory") {
        ...
        if (existingFavAnim) {
            ...
            document.getElementById(`MDLSBCategoryDivTexts2-favorites`).innerHTML=`Favorites`;
            var xhr = new XMLHttpRequest();
            xhr.open("POST", `https://${GetParentResourceName()}/callback`, true);
            xhr.setRequestHeader('Content-Type', 'application/json');
            xhr.send(JSON.stringify({action: "saveFavAnims", favoriteAnimations: favoriteAnimations}));
        } else {
        ...
        xhr.open("POST", `https://${GetParentResourceName()}/callback`, true);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.send(JSON.stringify({action: "playAnim", id: Number(name2), category: name3}));
    }
}
...
function updateContent(emotes) {
    ...
    emotes.forEach((item, index) => {
        ...
        if (existingAnim) {
            ...
            if (item.category === "general") {category = "emotes"};
...
function addAnimToQuick(data, id) {
    ...
    xhr.open("POST", `https://${GetParentResourceName()}/callback`, true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(JSON.stringify({action: "saveQuickAnims", quickAnimations: quickAnimations}));
    playSound("CLICK_BACK", "WEB_NAVIGATION_SOUNDS_PHONE");
}
