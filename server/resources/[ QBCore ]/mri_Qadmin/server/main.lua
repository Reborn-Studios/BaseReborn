...
RegisterNetEvent("mri_Qadmin:server:RequestVehiclesInfo", function()
    ...
    if AdminPanel.HasPermission(src, "vehiclesinfo") then
        local results = Compat.GetVehiclesList() or {}
        if results ~= nil then
...
RegisterNetEvent("mri_Qadmin:server:RequestItemsInfo", function()
    ...
    if AdminPanel.HasPermission(src, "itemsinfo") then
        local items = Compat.GetItemsList() or {}
        while not next(items) do
            items = Compat.GetItemsList()
            Wait(500)
        end
...
AdminPanel.HasPermission = function(targetId, permName)
    ...
    if Player then
        for k,v in pairs(Config.Permissions) do
            if QBCore then
                if QBCore.Functions.HasPermission(targetId, "Owner")  then
                    return true
                end
...
RegisterNetEvent("mri_Qadmin:server:RequestServerMetrics", function()
    ...
    if AdminPanel.HasPermission(src, "servermetrics") then
        ...
        if QBCore then
            results = MySQL.query.await("SELECT DISTINCT `steam` FROM `"..Config.DB.CharactersTable.."`", {})
        end
