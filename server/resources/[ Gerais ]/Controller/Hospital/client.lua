					...
					if distance <= 1.5 and IsControlJustPressed(1,38) and HPServer.checkServices() then
						if GetEntityHealth(ped) < maxHealth then
							...
							if checkBusy >= #v.beds then
								TriggerEvent("Notify","Tratamento","Todas as macas estão ocupadas, aguarde.","ambulance",5000)
							end
						else
							TriggerEvent("Notify","Tratamento","Você está bem de saude.","ambulance",5000)
						end
					end
				end
			end
...
AddEventHandler('tratamento-macas',function()
	...
	TriggerEvent("Notify","Tratamento","Tratamento concluido.","ambulance",3000)
...
AddEventHandler("tratamento",function()
	...
	tratamento = true
	TriggerEvent("Notify","Tratamento","Tratamento iniciado, aguarde a liberação do <b>profissional médico.</b>.","ambulance",8000)
	...
	if tratamento then
		...
		until GetEntityHealth(ped) >= maxHealth or GetEntityHealth(ped) <= 101
			TriggerEvent("Notify","Tratamento","Tratamento concluido.","ambulance",8000)
			tratamento = false
			TriggerEvent("vrp_survival:desbugar")
	end
end)
