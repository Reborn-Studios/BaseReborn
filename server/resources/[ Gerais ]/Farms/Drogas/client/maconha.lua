CreateThread(function()
	while true do
		local will = 1000
		local closestFarm, loc = GetClosestFarm('maconha')
		if closestFarm and loc then
			...
			if data and not InProcess then
				...
				if distance <= 5.0 then
					...
					if distance <= 2.0 and IsControlJustPressed(0,38) and Drugs.checkPermission(Farms.maconha[closestFarm]['perm']) then
						if data['id'] == 1 then
