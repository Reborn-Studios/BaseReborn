...
-- AÇÃO 
-------------------------------------------------------------------------------------------------
CreateThread(function() -- PEGAR PAPEL
	while true do
		local sleep = 800
		...
		if GetDistanceBetweenCoords(playerCds.x, playerCds.y, playerCds.z, x,y,z, true) <= 2 and not papel then
			...
			if GetDistanceBetweenCoords(playerCds.x, playerCds.y, playerCds.z,x,y,z, true) <= 2 and not papel then
				if IsControlJustPressed(0,38) and Lavagem.checkItens() then
					papel = true
...
CreateThread(function() -- COLOCAR PAPEL
	while true do
		local sleep = 800
		local playerCds = GetEntityCoords(PlayerPedId())
...
CreateThread(function() -- PEGAR NOTAS FALSAS
	while true do
		local sleep = 800
		local playerCds = GetEntityCoords(PlayerPedId())
...
CreateThread(function() -- Cortar NOTAS FALSAS
	while true do
		local sleep = 800
		local playerCds = GetEntityCoords(PlayerPedId())
		...
		if GetDistanceBetweenCoords(playerCds.x, playerCds.y, playerCds.z,x,y,z, true) <= 2 and pegarnota and not colocarnota then
			...
			if GetDistanceBetweenCoords(playerCds.x, playerCds.y, playerCds.z,x,y,z, true) <= 2 and pegarnota and not colocarnota then
				if IsControlJustPressed(0,38) then
					...
					local netScene = NetworkCreateSynchronisedScene(x,y, z, targetRotation.x, targetRotation.y, targetRotation.z, 2, false, false, 1148846080, 0, 1.3)
...
CreateThread(function() -- EMBALAR NOTAS FALSAS
	while true do
		local sleep = 800
		local playerCds = GetEntityCoords(PlayerPedId())
