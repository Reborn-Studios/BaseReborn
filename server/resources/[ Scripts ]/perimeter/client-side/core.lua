		...
		for Selected,v in pairs(Blip) do
			if Perimeter[Selected] then
				if #(Coords - GetBlipCoords(v)) <= Perimeter[Selected].Distance and not Notify then
					Notify = Selected
					TriggerEvent("Notify","Atenção","Você entrou em uma área onde a circulação está restrita, pedimos que se dirija a um local autorizado ou siga as orientações para garantir sua segurança e a de todos, agradecemos pela compreensão.",10000)
				elseif Notify == Selected and #(Coords - GetBlipCoords(v)) > Perimeter[Selected].Distance then
					TriggerEvent("Notify","Aviso","Você saiu de uma área onde a circulação estava restrita, agradecemos por seguir as orientações e garantir a segurança de todos, se precisar de mais informações estamos à disposição.",10000)
					Notify = false
				end
			end
		end
