...
RegisterNetEvent("hud:setClientUserId")
AddEventHandler("hud:setClientUserId", function (userID_to_set)
    if userID_to_set then
        ClientUserId = userID_to_set
    end
end)
...
Citizen.CreateThread(function()
    ...
    SendNUIMessage({ type="set_id",  value = ClientUserId or 0})
...
AddEventHandler('hudActived', function(val)
    while true do
        Citizen.Wait(0)
    ...
    SendNUIMessage({ type = "set_force_hide", value = not val })
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if health > 101 then
			if myhunger >= 10 and myhunger <= 20 then
				SetFlash(0,0,500,1000,500)
				SetEntityHealth(ped,health-1)
			elseif myhunger <= 9 then
				SetFlash(0,0,500,1000,500)
				SetEntityHealth(ped,health-2)
			end

			if mythirst >= 10 and mythirst <= 20 then
				SetFlash(0,0,500,1000,500)
				SetEntityHealth(ped,health-1)
			elseif mythirst <= 9 then
				SetFlash(0,0,500,1000,500)
				SetEntityHealth(ped,health-2)
			end
		end
...
RegisterNetEvent('hud:client:UpdateSettings')
AddEventHandler('hud:client:UpdateSettings', function(data)
...
function CinematicShow(bool)
    ...
    TriggerEvent('hud:client:UpdateSettings', preferencesData)
    if bool then
...
Citizen.CreateThread(function()
    while true do
		...
		_hash1 = GetStreetNameFromHashKey(var1);
		_hash2 = GetStreetNameFromHashKey(var2);
...
Citizen.CreateThread(function()
    if not Config.DisplayRealTime then
        while true do
            Citizen.Wait(1000)
            local hours = GlobalState.clockHours -- GetClockHours()
            local minutes = GlobalState.clockMinutes -- GetClockMinutes()
...
if Config.EnableSeatbelt then
    ...
    RegisterNetEvent('hud:client:EjectPlayer')
    AddEventHandler('hud:client:EjectPlayer', function(velocity)
	    ...
	    while true do
	        ...
	        if car ~= 0 and (wasInCar or IsCar(car)) then
                ...
                if speedBuffer[2] and GetEntitySpeedVector(car, true).y > 1.0  and speedBuffer[1] > 15 and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.255) then			   
                    ...
                    if #seatPlayerId > 0 then TriggerServerEvent("hud:server:EjectPlayer", seatPlayerId, velBuffer[2]) end                    
	        	end
...
if Config.EnableEngineToggle then
    ...
    RegisterNetEvent('hud:client:ToggleEngine')
    AddEventHandler('hud:client:ToggleEngine', function(toggle)
        local vehicle = GetVehiclePedIsIn(playerPed, false)
        if vehicle == 0 or GetPedInVehicleSeat(vehicle, -1) ~= playerPed then return end
        if not toggle then
            Config.Notification(Config.Notifications["engine_off"]["message"], Config.Notifications["engine_off"]["type"])
