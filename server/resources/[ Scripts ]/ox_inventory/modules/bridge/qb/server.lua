...
function server.buyLicense(inv, license)
	...
	elseif Inventory.GetItem(inv, 'dollars', false, true) < license.price then
		return false, 'can_not_afford'
	end

	Inventory.RemoveItem(inv, 'dollars', license.price)
	...
	if type(items) == 'table' then
		...
		if player then
			for name in pairs(server.accounts) do
				...
				if not hasThis then
					local amount = player.Functions.GetMoney(name == 'dollars' and 'cash' or name)
