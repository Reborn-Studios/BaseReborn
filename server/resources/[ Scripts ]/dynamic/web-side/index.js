  ...
  window.addEventListener("message", function (event) {
    ...
    if (item["addbutton"] == true) {
      if (item.id == false || null) {
        ...
        var b =
          `<button id="normalbutton-${normalButtons}" data-trigger="` +
          item["trigger"] +
          `" data-parm="` +
          item["par"] +
          `" data-server="` +
          item["server"] +
          `" class="btn normalbutton"><div class="title2">` +
          item["title"] +
      ...
      var aa =
        ...
        `"class="b btn"><i class=" ` +
        item["icons"] +
        ` icon1"></i><div class="title">` +
        item["title"] +
        `</div><div class="description" >` +
        item["description"] +
        `</div><i class="fas fa-chevron-right" style="float:right;margin-top:-10%"></i></button>`;
      $(".container").append(aa);
      submenus.push(aa);
    }
