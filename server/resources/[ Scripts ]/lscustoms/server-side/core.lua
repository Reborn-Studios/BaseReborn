...
function Creative.Permission(Index)
	...
	local _, Amount = vRP.NumPermission("mecanico.permissao")
	-- if Amount == 0 then
	-- 	return true
	-- end
...
function Creative.Save(Model,Plate,Initial)
	...
	if Passport and (Price <= 0 or vRP.PaymentFull(Passport,Price,true)) then
		...
		if OtherPassport then
			local Consult = vRP.GetSrvData("custom:"..OtherPassport..":"..Model,true)
			...
			vRP.SetSrvData("custom:"..OtherPassport..":"..Model,json.encode(Consult),true)
		end
